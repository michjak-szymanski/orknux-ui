...
export interface UnsavedFunctionDialogProps {
  /** What the function is called, or null while there is nothing to ask about. */
  functionName: string | null;
  /** Whether this function has ever been stored, which changes what saving means. */
  creating: boolean;
...
/**
 * The one word the function editor never said before walking away.
 *
 ...
 * presses it once, and somebody who did not gets their code back by pressing
 ...
 */
export function UnsavedFunctionDialog({
  functionName,
  creating,
  onLeave,
  onSaveAndLeave,
  onStay,
}: UnsavedFunctionDialogProps) {
  ...
  useEffect(() => {
    ...
    if (functionName !== null && !dialog.open) {
      setError(null);
      setSaving(false);
      dialog.showModal();
    } else if (functionName === null && dialog.open) {
      dialog.close();
    }
  }, [functionName]);
      ...
      <div className={styles.body}>
        ...
        <div className={styles.warning}>
          ...
          <p className={styles.warningMessage}>
            {creating ? (
              <>
                <strong>&quot;{functionName}&quot;</strong> has not been created yet. Leaving now throws away
                the code and the details on this screen, and there is no way back to them.
              </>
            ) : (
              <>
                <strong>&quot;{functionName}&quot;</strong> has changes the server has not been told about.

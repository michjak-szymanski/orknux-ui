...
it, and how Quick actions names it — and `src/routes.tsx` maps each of those paths
...
**The shell.** The box in the top bar is **Quick actions**: it searches the
pages and the workspace's own things — workflows, actions, agents, issues and
the rest — opens what was chosen, and offers a few things to start. The
...
browser rather than against the account. Quick actions, Save, Format, Turn node,
Undo and Redo each have one.
